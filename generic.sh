...
function extract_karaf() {
    ...
    if [ $? -ne 0 ];then echo -ne "Error while archiving karaf \n";return 1;fi
    echo -ne "Copying Properties to $final_karaf\n"
    cp -rf ${karaf_dir}/${sp_dir}/*.properties $final_karaf
    if [ $? -eq 0 ];then echo -ne "Karaf APC done\n";else echo -ne "Error While copying\n";fi 
}
...
function extract_nfs() {
    ...
    if [ $? -ne 0 ];then echo -e "Error while archiving karaf nfs \n";return 1;fi
    echo -ne "Copying Properties to $final_nfs\n"
    cp -rf ${nfs_dir}/${sp_dir}/*.properties $final_nfs
    if [ $? -eq 0 ];then echo -ne "Karaf NFS done\n";else echo -ne "Karaf NFS: Error While copying\n";fi 
}
...
function extract_hazelcast() {
    ...
    if [ $? -eq 0 ];then echo -ne "Hazel done\n";else echo -ne "Hazel: Error While archiving\n";fi 
}
...
function extract_storm() {
    ...
    if [ $? -eq 0 ];then echo -ne "Storm done\n";else echo -ne "Storm: Error While archiving\n";fi 
}

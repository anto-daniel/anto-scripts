...
user = ConfigSectionMap("credentials")['username']
passwd =  ConfigSectionMap("credentials")['password']
host = ConfigSectionMap("hostinfo")['host']
port = ConfigSectionMap("hostinfo")['port']
dbname = ConfigSectionMap("db")['db']
...
def save_attr(asset):
    ...
    for addattr in add_attributes(asset):
        tmp1.append(addattr)
#    for att in uniq(tmp1):
#        print docid+": Adding attribute :"+att
#        doc[att] = { "doc": "", "type":"" }
#        db.save(doc)
    t = []
    for att in tmp1:
